...
# 2016-10-07 - Newcomer - Changed the loop to exit when it finds all disks it was asked to.
...
DISKS=$(egrep "REMOVED|DEGRADED|UNAVAIL|FAULTED|OFFLINE" zfs/zpool-status-dv.out | grep c[0-9]*t[0-9A-F]*d[0-9] | sed -e 's/.*\(c[0-9]\+t[0-9A-Z]\+d0\)[s0]\?.*/\1/')
            ...
            if [[ ! -z $(echo $COL|awk '{print $1}') ]]; then
                ...
                for j in $(echo $DISKS); do
                    ...
                    if [[ -e $COL/enclosures/nmc-c-show-lun-slotmap.out ]]; then
                        LOC=$(grep $DID $COL/enclosures/nmc-c-show-lun-slotmap.out | awk '{print $2, "slot:"$3}')
